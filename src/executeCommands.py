...
def _REMOVE_OLD_LOGS(
    ...
    for logFile in itemLocation.glob(
...
def _EXECUTE_ITEM(
    ...
    print(
        ...
        f"{projectID}/{item['itemID']}"

    )

    startTime = time.time()

    for commandIndex, command in enumerate(
        ...
        logFile = (
            ...
            f"log.executeCommand{commandIndex:02d}.txt"
