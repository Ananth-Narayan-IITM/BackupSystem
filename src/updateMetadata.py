...
def UPDATE_METADATA(
    ...
    metadataDictionary, metadataPath = _READ_METADATA(
...
def _UPDATE_PROJECTS(
    ...
    for projectID in backupSummary[

            "successfulProjects"

    ]:

        if projectID not in metadataDictionary[
...
def _UPDATE_ITEMS(
    ...
    for itemID in backupSummary[

            "successfulItems"

    ]:

        if itemID not in metadataDictionary[
...
def _PRINT_METADATA_SUMMARY(
    ...
    print(
        ...
        f"{len(backupSummary['successfulProjects'])}"
